...
export function authFromMiniprogramOrderUser(
	...
	const cookie = ensureUidCookie(overrides.cookie ?? orderUser.cookie ?? "", uid);
...
export async function postLuckinJson(
	...
	try {
		...
		throw new MiniprogramClientError("Invalid Luckin miniprogram response", 502);
	}
}
...
function buildSignedFormBody(payload: Record<string, unknown>, auth: MiniprogramAuth) {
